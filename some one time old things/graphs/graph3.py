...
# Plot the line
ax.plot(years, popularity, 'o:r', color='black', linewidth=1, label="Popularity Trend")

# Add colored dots for each year based on genre
for year, pop, genre in zip(years, popularity, genres):
    ax.plot(year, pop, 'o', color=colors[GenreNamesList.index(genre)], label=genre if genre not in ax.get_legend_handles_labels()[1] else "")
